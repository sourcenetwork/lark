...
impl WalReplayIter {
    ...
    /// same filesystem the database was written to. Reading the real
    /// one instead means a database on any other backend cannot reopen
    /// at all, which is what `MemEnv` and OPFS both hit.
