...
//!
//! Every capability lark's `Env` reports for a target is measured here
...
/// Probe every host primitive lark's `Env` depends on, under `root`.
///
...
fn write_then_read(path: &Path) -> std::io::Result<()> {
    fs::write(path, b"lark-probe-payload")?;
    let back = fs::read(path)?;
    if back != b"lark-probe-payload" {
