//! `regolith-bench` - a simple benchmark driver for regolith, modeled
//! after the workloads in db_bench.
//!
//! ```sh
//! cargo run --release -p regolith-bench -- \
...
fn report_full(name: &str, elapsed: Duration, ops: u64, bytes: u64, json: bool) {
    ...
    if json {
        // Hand-rolled JSON so regolith-compare can parse without
