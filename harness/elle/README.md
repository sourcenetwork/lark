...
PR 1 lands deliberately red. Measured with the commands above:

| History | Engine | Checker model | Verdict |
| --- | --- | --- | --- |
| `--isolation read-committed --keys 1` | `TransactionDb` | `read-committed` | `false` (`incompatible-order`) |
| `--isolation read-committed --keys 4` | `TransactionDb` | `strict-serializable` | `false` (`G0`, `G-nonadjacent-item`, `incompatible-order`) |
| `--model rw-register --isolation read-committed` | `TransactionDb` | `strict-serializable` | `false` |
| `--isolation repeatable-read` | `OptimisticTransactionDb` | `snapshot-isolation` | `true` |

The last row is the control: the same workload, the same engine, the
optimistic flavor, and Elle accepts it. That isolates the failure to the
pessimistic flavor rather than to the harness.

`Transaction::get_for_update` acquires the key lock but then reads at
the sequence number captured when the transaction began, and a
pessimistic commit applies its buffered writes unconditionally
(`src/transaction.rs`, `TxMode::Pessimistic` in `commit_inner`). Two
transactions therefore serialize on the lock and still both read the
pre-lock value, and the second one's read-modify-write clobbers the
first.

On a single hot key, three seeded runs of

```sh
./target/release/elle-gen --isolation read-committed \
  --threads 8 --txns 50 --keys 1 --seed <n> --out h.json --dir db
```

committed 520, 496 and 548 appends `ok` and carried 65, 63 and 76 of
them into the final read: 87.5%, 87.3% and 86.1% of committed updates
lost. The exact rate moves with scheduling; every run measured so far
has lost more than 80%.

The two write-ahead-log faults do not produce a history at all. After
either one, `Db::open` fails outright:

```text
fault: tore the final 32 bytes of db/wal/wal_000004.log at offset 1924 (mark 0)
elle-gen: recovery failed after '...': corruption: WAL checksum mismatch
fault: truncated db/wal/wal_000004.log from 1824 to 912 bytes (mark 0)
elle-gen: recovery failed after '...': corruption: truncated WAL record data
```

`Wal::replay` propagates a checksum mismatch or a short read from any
record, including the last one (`src/engine/wal.rs`), and
`LarkEngine::recover` propagates that out of `Db::open`
(`src/engine/mod.rs`). A torn or truncated tail record is the ordinary
artifact of a crash during a write, so a database that will not reopen
after one is a durability failure in its own right. The harness reports
it and stops rather than inventing read-back records for reads that
never ran. `--faults kill` is unaffected: lark recovers from it and the
history is generated normally.
